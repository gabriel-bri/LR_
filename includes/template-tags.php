...
function extra_get_post_rating_stars( $post_id = 0 ) {
	...
	$output = '<span class="rating-stars" title="' . esc_attr( sprintf( __( 'Votos: %0.2f', 'extra' ), $rating ) ) .'">' . extra_make_mini_stars( $rating ) . '</span>';
	return $output;
}
...
function extra_the_post_rating_stars_with_rating_count( $post_id = 0 ) {
	...
	printf(
		...
		esc_html( sprintf( _n( '%d votos', '%d votações', $rating_count ), $rating_count ) )
	);
}
...
function extra_rating_stars_display( $post_id = 0 ) {
	...
	if ( $rating = extra_get_user_post_rating( $post_id ) ) {
		...
		$title = esc_html__( 'Voto:', 'extra' );

		$output = '<p id="rate-title" class="rate-title">' . esc_html__( 'Voto:', 'extra' ) . '</p>';
...
function extra_make_fixed_stars( $rating ) {
	...
	for ( $x = 1; $x <= 5; $x++ ) {
		...
		$output .= '<img src="' . $src . '" class="rating-star '. $class . ' rating-star-' . $x . '" alt="' . esc_attr__( "Estrela", "extra" ) . '" />' . "\n";
	}
...
function extra_header_search_field( $echo = true ) {
	$output = sprintf(
		...
		esc_url( home_url( '/' ) ),
		esc_attr_x( 'BUSCAR', 'campo_busca', 'extra' ),
		get_search_query(),
		esc_attr_x( 'Buscar por:', 'buscar', 'extra' )
	);
...
function extra_get_post_author_link( $post_id = 0 ) {
	...
	$link = sprintf(
		...
		esc_attr( sprintf( __( 'Post por %s' ), $author->display_name ) ),
...
function extra_post_share_links( $echo = true ) {
	...
	foreach ( $networks as $network ) {
		...
		$share_title = sprintf( __( 'Compartilhar "%s" via %s', 'extra' ), $title, $network->name );
...
function extra_get_sitemap_page_options( $post_id = 0 ) {
	...
	$sections = array(
		'pages'        => esc_html__( 'Páginas', 'extra' ),
		'categories'   => esc_html__( 'Categorias', 'extra' ),
		'tags'         => esc_html__( 'Tags', 'extra' ),
		'recent_posts' => esc_html__( 'Posts recentes', 'extra' ),
		'archives'     => esc_html__( 'Arquivos', 'extra' ),
		'authors'      => esc_html__( 'Autores', 'extra' ),
	);
		...
		case 'authors':
			...
			foreach ( $authors as $author ) {
				$items .= sprintf(
					...
					esc_attr( sprintf( __( 'Posts por: %s', 'extra' ), $author->display_name ) ),
					esc_html( $author->display_name )
				);
			}
			break;
...
function extra_ajax_loader_img( $echo = true ) {
	$img = '<img src="' . esc_url( get_template_directory_uri() ) .'/images/pagination-loading.gif" alt="' . esc_attr__( "Carregando", "extra" ) . '" />';
	if ( $echo ) {
		echo $img;
	} else {
		return $img;
